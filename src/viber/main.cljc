(ns viber.main
  "Kotodama WASM entrypoint for the Viber clean-room actor (L4) — Clojure port.
  ...
  facts (`viber.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"viber.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "viber-compat" :tier tier :entities entities} 200])
